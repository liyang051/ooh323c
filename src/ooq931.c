...
EXTERN int ooQ931Decode (OOH323CallData *call,
   ...
   (OOH323CallData* call, ASN1OCTET *msgbuf, ASN1UINT msglen)
{
   ...
   ret = ooQ931Decode (call, &q931Msg, msglen, msgbuf, FALSE);
   if(ret != OO_OK)
   {
                        ...
                        ASN1OCTET* msgbuf, size_t size)
{
   ...
   if(pq931Msg->messageType == Q931SetupMsg){
      ...
      OOTRACEERR3("ERROR: Unknown Q931 message type. (%s, %s)\n",
                  call->callType, call->callToken);
      return OO_FAILED;
   }
...
#ifndef _COMPACT
   if(msgbuf[0] != OOFacility)
     ooQ931PrintMessage (call, (ASN1OCTET*)msgbuf+5, len-4);
   else
     ooQ931PrintMessage (call, (ASN1OCTET*)msgbuf+8, len-4);
#endif
   return OO_OK;
}
   ...
   ASN1UINT *fsCount, ASN1DynOctStr **fsElem)
{
      ...
      pCall->remoteFastStartOLCs.count>0)
   {
      ...
      for(i=0, j=0; i<(int)pCall->remoteFastStartOLCs.count; i++)
      {
         ...
         pFS[j].data = encodeGetMsgPtr(pctxt, (int*)&(pFS[j].numocts));


         /* start print call */
         setPERBuffer(pctxt, (ASN1OCTET*)pFS[j].data, pFS[j].numocts, 1);
...
(OOCTXT* pctxt, OOH323CallData* call, int msgType)
{
   Q931Message* q931msg;

   if (ooCreateQ931Message (&q931msg, msgType) != OO_OK) {
      OOTRACEERR1 ("Error allocating memory for Q.931 message\n");
      return 0;
   }
   ...
   if (0 == q931msg->userInfo) {
      OOTRACEERR1 ("Error allocating memory for H323_UserInformation\n");
...
int ooSendStatus (OOH323CallData* call)
{
   ...
   OOTRACEDBGC3 ("Building Status-UUIE (%s, %s)\n",
                 call->callType, call->callToken);
   ...
   if (0 == pStatusUUIE) {
      OOTRACEERR1 ("ERROR: Memory - ooSendStatus - pStatusUUIE\n");
      memFreePtr (pctxt, q931msg);
      return OO_FAILED;
   }
...
int ooSendFacility(OOH323CallData *call)
{
   ...
   if (0 == pQ931Msg) {
      OOTRACEERR1 ("ERROR: Memory - ooSendFacility - userInfo\n");
      return OO_FAILED;
   }
...
int ooH323ForwardCall(char* callToken, char *dest)
{
   ...
   if(!ooUtilsIsStrEmpty(call->pCallFwdData->ip))
   {
      ...
      ooSocketConvertIpToNwAddr
        (call->pCallFwdData->ip, fwdCallSignalIpAddress->ip.data,
         sizeof(fwdCallSignalIpAddress->ip.data));
